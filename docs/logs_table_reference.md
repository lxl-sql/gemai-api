...
`request_id`、`upstream_request_id`、`request_ip`、`request_domain`、`user_agent`
和 `content` 只支持日志明细查询，不是分钟聚合维度。统计接口收到这些非空参数时会明确返回
“当前筛选条件不支持统计”，前端也不会发起统计请求，避免静默返回全站总计。
...
- `/self/stat` 的用户名始终使用精确匹配；管理员 `/stat` 保留 `%` 模式搜索能力。
- 统计数据按分钟预聚合，目标覆盖最近 7 天（保留边界额外留 1 小时缓冲，每整点清理一次过期分钟桶；即使回填被停用清理也照常执行）。无维度筛选的管理员整分钟统计读取 `log_stat_minute_totals`（每分钟固定一行），带任意维度筛选以及个人统计读取 `log_stat_rollups`；首尾不足一分钟的碎片和水位之后的短尾部从原始日志补算。
- `log_stat_minute_totals` 使用独立的 `minute_total_v1` 覆盖状态。升级后先生成最近分钟，再从现有维度聚合表由近及远回填；尚未覆盖的范围返回“统计数据正在初始化”，不会回退执行全维度大范围 SUM。
- `start_timestamp=0` 等同于 `LOG_QUERY_DEFAULT_DAYS`（默认 7 天）；该配置为 0 时收敛为“当前已覆盖的全部聚合数据”，绝不回退到原始日志全表扫描。
- 历史回填**由近及远**执行：部署后先覆盖“近期重算窗口”（生产默认 10 分钟），更长时间窗随回填推进逐步可查；接口不会把部分区间静默伪装成完整总额，查询起点尚未覆盖时返回“统计数据正在初始化”。
...
- `log_stat_setting.enabled`：是否启用实时分钟聚合，默认 `true`。紧急停用后统计接口明确返回不可用，不回退扫描原始日志。
- `log_stat_setting.backfill_enabled`：是否回填保留范围内的历史数据，默认 `true`。
- `log_stat_setting.recent_minutes`：每分钟重新聚合的近期窗口，范围 5–30，默认 `10`。用于吸收延迟日志和 master 切换；1 master + 多 slave 生产集群仍必须统一 NTP。

设置持久化在共享主库 `options` 表，并按 `SYNC_FREQUENCY`（默认 60 秒）同步到各节点。
master 首次启动新版本时只补写缺失的默认项，不覆盖已经存在的设置。
同名 `LOG_STAT_ROLLUP_ENABLED`、`LOG_STAT_BACKFILL_ENABLED` 和
`LOG_STAT_ROLLUP_RECENT_MINUTES` 环境变量仅作为没有数据库设置时的启动默认值，现有部署无需立即删除。

**高级保护参数（环境变量，可不配置，使用内置生产默认值）**：
