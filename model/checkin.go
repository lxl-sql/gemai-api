...
func UserCheckin(userId int) (*Checkin, error) {
	...
	// 三种数据库统一走事务，保证签到记录与额度入账原子提交；
	// SQLite 通过 per-user 互斥锁补偿缺失的行级锁。
...
func userCheckinWithTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 步骤2: 在事务中增加用户赠送额度
		if _, err := CreditGiftQuotaTx(tx, userId, quotaAwarded, QuotaTransactionRef{
			Type:           QuotaTransactionTypeGift,
			Source:         "checkin",
			ReferenceType:  "checkin",
			ReferenceID:    checkin.CheckinDate,
			IdempotencyKey: "checkin:" + checkin.CheckinDate + ":" + fmt.Sprintf("%d", userId),
		}); err != nil {
