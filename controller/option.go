...
func UpdateOption(c *gin.Context) {
	...
	if err := system_setting.ValidateTokenUsageSourceOption(option.Key, option.Value.(string)); err != nil {
	...
	// 仅记录被修改的配置项 Key，不记录 Value：部分配置含密钥/密码等敏感信息。
	model.RecordOperationLog(c, model.OpActionOptionUpdate, "option", option.Key, true, map[string]interface{}{
		"key": option.Key,
	})
	c.JSON(http.StatusOK, gin.H{
		"success": true,
