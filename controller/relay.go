...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	if types.IsClientDisconnectedError(err) {
		logger.LogWarn(c, fmt.Sprintf("channel #%d: client disconnected (status %d): %s", channelError.ChannelId, err.StatusCode, common.LocalLogPreview(err.Error())))
		return
	}
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		if c.Request != nil {
			...
			adminInfo["request_host"] = c.Request.Host
		}
