    ...
    try {
      await withVerification(
        async () => {
          ...
          if (!result.success) {
            throw new Error(
              result.message || t(ERROR_MESSAGES.STATUS_UPDATE_FAILED)
            )
          }
        ...
        {
          title: t(isEnabled ? 'Disable API Key' : 'Enable API Key'),
          description: t(
            'Confirm your identity before changing this API key status.'
          ),
        }
      )
    } catch (error) {
  ...
  return (
    <div className='-ml-1.5 flex items-center gap-1'>
      ...
      >
        ...
        <DropdownMenuItem onClick={() => void requestRotate(apiKey)}>
          {t('Rotate API Key')}
          <DropdownMenuShortcut>
            <RotateCcw size={16} />
          </DropdownMenuShortcut>
